...
export const Arrow: React.FC<ArrowProps> = ({ connection }) => {
  ...
  const createPath = () => {
    ...
    if (fromSide === 'bottom' && toSide === 'top') {
      ...
      // From left side to top (rare)
      const offsetX = start.x - 30;
      return `M ${start.x} ${start.y} L ${offsetX} ${start.y} L ${offsetX} ${end.y} L ${end.x} ${end.y}`;
  ...
  const getLabelPosition = () => {
    ...
    if (fromSide === 'bottom' && toSide === 'top') {
      ...
      const offsetX = start.x - 30;
      return { x: offsetX, y: (start.y + end.y) / 2 };
    } else if (fromSide === 'bottom' && toSide === 'right') {
      const offsetY = start.y + 30;
      return { x: (start.x + end.x) / 2, y: offsetY };
    }
